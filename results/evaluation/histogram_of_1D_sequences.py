...
quasi_crystal, slope, _ = quasi_crystal_1d(lattice_spacing = 1, slope = generate_irrational(upper_limit = 2), acceptance_window = 10, number_of_points = 100)
random_step = random_step_1d(step_size = 1, positive_probability = 0.5, number_of_points = 100)
gaussian_step = gaussian_step_1d(step_size = 1, std_dev = 0.1, number_of_points = 100)

# Generating histograms
histogram(quasi_crystal, title = f"1D Quasi Crystal Histogram, slope = {slope}", lattice_spacing = 1, bins = 20, log = False)
plt.savefig(os.path.join(results_path, "quasi_crystal_histogram.png"))
plt.clf()

histogram(random_step, title = "1D Random Step Histogram", lattice_spacing = 1, bins = 20, log = False)
plt.savefig(os.path.join(results_path, "random_step_histogram.png"))
plt.clf()

histogram(gaussian_step, title = "1D Gaussian Step Histogram", lattice_spacing = 1, bins = 20, log = False)
plt.savefig(os.path.join(results_path, "gaussian_step_histogram.png"))
plt.clf()